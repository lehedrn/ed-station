...
| 移动端 | UniApp + uView UI 2.x（二期） | 移动办公、事项查询（暂不实现） |
...
> - **暂不实现**：移动端应用、事项版本对比、事项办件统计、网上申报对接、审核流程
...
**权限规则**：
- 同部门/同机构可查看，仅创建人可维护本人创建的事项
- 系统管理员拥有所有权限
- 数据权限：基于 BladeX 多租户隔离 (`tenant_id`)，支持部门级数据权限
...
| 实施编码 | 文本输入 | 是 | 国家统一编码，格式：XXXXXXXX + 8 位数字 | `implement_code` |
| 事项类别 | 下拉选择 | 是 | 数据来源：`affair_type` 字典 | `affair_type` |
| 事项子类 | 下拉选择 | 是 | 数据来源：`affair_sub_type` 字典 | `affair_sub_type` |
| 实施部门 | 树形选择 | 是 | 选择部门（数据权限） | `dept_id` |
| 法定时限 | 数字输入 | 是 | 单位：工作日，0 表示即时办结 | `legal_limit` |
| 承诺时限 | 数字输入 | 是 | 单位：工作日，≤法定时限 | `promise_limit` |
| 办理条件 | 富文本编辑 | 是 | 支持 HTML 格式 | `handle_condition` |
| 所需材料 | 动态列表 | 是 | 材料清单（子表） | - |
| 收费标准 | 单选框组 | 是 | 收费/不收费 | `charge_flag` |
| 收费依据 | 文本输入 | 条件必填 | 收费时必填 | `charge_basis` |
| 办理形式 | 多选框组 | 是 | 窗口办理/网上办理/自助终端 | `handle_form` |
| 到现场次数 | 数字输入 | 是 | 默认 0（全程网办） | `visit_times` |
| 办理结果 | 文本输入 | 是 | 如：证照/批文/决定 | `handle_result` |
| 结果样本 | 文件上传 | 否 | word/pdf，最多 3 个 | `result_sample` |
| 咨询方式 | 文本输入 | 否 | 咨询电话/地址 | `consult_info` |
...
1. **唯一性校验**：同一实施机构下，事项名称和实施编码不能重复
2. **时限规则**：承诺时限 ≤ 法定时限，承诺时限为 0 表示即时办结
3. **状态规则**：
   - 正常状态：可编辑、可删除、可下架
   - 已下架状态：可编辑、可删除、可重新发布
   - 已删除状态：逻辑删除，不显示
4. **必填规则**：收费标准选择"收费"时，收费依据必填
5. **材料管理**：至少需要添加一项所需材料
...
| 表名 | 说明 | 继承 | 多租户 |
|------|------|------|--------|
| `blade_affair` | 事项主表 | TenantEntity | 是 |
| `blade_affair_material` | 事项材料关联表 | BaseEntity | 是 |
...
| `implement_code` | varchar(32) | 实施编码 | - |
| `affair_type` | varchar(32) | 事项类别 | `affair_type` |
| `affair_sub_type` | varchar(32) | 事项子类 | `affair_sub_type` |
| `dept_id` | bigint | 实施部门 ID | - |
| `legal_limit` | int | 法定时限（工作日） | - |
| `promise_limit` | int | 承诺时限（工作日） | - |
| `handle_condition` | text | 办理条件 | - |
| `charge_flag` | varchar(1) | 是否收费（0:否 1:是） | `yes_no` |
| `charge_basis` | varchar(500) | 收费依据 | - |
| `handle_form` | varchar(64) | 办理形式（多选逗号分隔） | `handle_form` |
| `visit_times` | int | 到现场次数 | - |
| `handle_result` | varchar(200) | 办理结果 | - |
| `result_sample` | varchar(1000) | 结果样本附件 URL | - |
| `consult_info` | varchar(500) | 咨询方式 | - |
| `remark` | text | 备注说明 | - |
...
| `is_required` | varchar(1) | 是否必收（0:否 1:是） | `yes_no` |
...
| `affair_sub_type` | 事项子类 | `0101`→企业法人许可，`0102`→个体工商户许可，`0501`→社保服务，`0502`→医保服务... |
| `handle_form` | 办理形式 | `01`→窗口办理，`02`→网上办理，`03`→自助终端办理，`04`→邮寄办理 |
...
-- 办理形式字典
INSERT INTO blade_dict (id, parent_id, code, dict_key, dict_value, sort)
VALUES (100008, 0, 'handle_form', '-1', '办理形式', 1);

INSERT INTO blade_dict (id, parent_id, code, dict_key, dict_value, sort)
VALUES (100009, 100008, 'handle_form', '01', '窗口办理', 1),
       (100010, 100008, 'handle_form', '02', '网上办理', 2),
       (100011, 100008, 'handle_form', '03', '自助终端办理', 3),
       (100012, 100008, 'handle_form', '04', '邮寄办理', 4);
...
blade_affair ──N:1── blade_dict (affair_sub_type)
blade_affair ──N:1── sys_dept (dept_id)
blade_affair ──N:1── sys_user (create_user, audit_user)
blade_affair ──1:N── blade_affair_audit_log
```
...
- 数据权限：基于 `tenant_id` 多租户隔离，支持部门级数据权限
- 同部门可查看，仅创建人可维护本人创建的事项
- 审核人员可跨部门审核

### 6.2 数据安全要求

- 敏感字段脱敏展示（列表显示）
- 文件上传格式限制：pdf/doc/docx，单个文件≤20M
- 操作日志完整记录（BladeX 自动记录）
- SQL 注入防护（BladeX 自动过滤）
- 实施编码格式校验（8 位字母 +8 位数字）
...
| 编号 | 问题 | 说明 | 确认结果 | 文档处理 |
|------|------|------|---------|---------|
| 1 | 事项类别方式 | 采用字典方式还是分类方式 | 1-A：采用简单字典方式（affair_type 字典） | 已更新至字段说明 |
| 2 | 审核流程 | 是否需要实现审核功能 | 2-A：简化版，无审核流程，事项新增后直接生效 | 已移除审核相关内容 |
| 3 | 材料管理 | 材料如何存储 | 3-B 修改版：子表只存 affair_id 和 attach_id，文件上传到 blade_attach 表 | 已更新至数据库设计 |
| 4 | 承诺时限单位 | 承诺时限使用什么单位 | 4-A：仅使用工作日为单位 | 已更新至字段说明 |
